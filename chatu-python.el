;;; chatu-python.el --- Chatu for Python  -*- lexical-binding: t -*-
...
;; script and open function for Python

;;; Code:

(require 'chatu-common)

(defun chatu-python-script (keyword-plist)
  "Open input.
KEYWORD-PLIST contains parameters from the chatu line."
  (let* ((path (plist-get keyword-plist :input-path))
         (path (chatu-common-with-extension path "py")))
    (format "python %s"
            (shell-quote-argument path))))

(defun chatu-python-open (keyword-plist)
  "Open input file.
KEYWORD-PLIST contains parameters from the chatu line."
  (interactive)
  (let* ((path (plist-get keyword-plist :input-path))
...
(provide 'chatu-python)

;;; chatu-python.el ends here
